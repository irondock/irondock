...
gulp.task('watch', ['inject'], function() {
  ...
  // Watch for changes in all files inside the vendor directory
  chokidar.watch('vendor', {ignoreInitial: true}).on('all', function(event, _path) {
    reloadBrowser(_path, event);
  });
...
gulp.task('serve', ['setenv:development', 'clean:tmp', 'appinfo', 'watch'], function() {
  ...
  browserSync.init({
    ...
    server: {
      baseDir: [options.paths.tmp, 'vendor', path.join(options.paths.src, 'assets'), path.join(options.paths.src, 'app')],
      routes: {
...
gulp.task('fonts', function () {
  return gulp.src($.mainBowerFiles().concat(['vendor/**/*']))
